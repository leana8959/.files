...
(fn has-words-before []
  ...
  (and
    (not= col 0)
    (nil?
      (let [l (. (vim.api.nvim_buf_get_lines 0 (- line 1) line true) 1)]
...
; Lazy loader ;
;;;;;;;;;;;;;;;
(require-then "luasnip.loaders.from_vscode" #($.lazy_load {:paths ["./snippets"]}))
      ...
      { ; Next or jump
        ...
        "<CR>" (cmp.mapping #(
            if
              (cmp.visible)
                (let
                  [confirm (cmp.mapping.confirm { :behavior cmp.ConfirmBehavior.Insert :select true})]
                  (confirm))
              ($))
          [:i :s])
        ...
        "<S-CR>" (cmp.mapping #(
            if
              (cmp.visible)
                (let
                  [confirm (cmp.mapping.confirm { :behavior cmp.ConfirmBehavior.Replace :select true})]
                  (confirm))
              ($))
          [:i :s])})
      ...
      (let
        ...
        [ {:name "luasnip"}
          ...
          { :name "buffer"
            :keyword_length 10
            :option
              {:enable_in_context #(of-filetype [:tex :markdown :typst] $)}}
          { :name "spell"
            :keyword_length 10
            :option
